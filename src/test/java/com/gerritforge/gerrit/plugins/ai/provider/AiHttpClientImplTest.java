...
public class AiHttpClientImplTest {
    ...
    AiCodeReviewException exception =
        assertThrows(
            ...
            () ->
                aiHttpClient.execute(
                    httpClientRequest,
                    status -> status == HttpServletResponse.SC_OK,
  ...
  public void shouldCallResponseHandlerWhenReturningOk() throws Exception {
    ...
    doReturn(HttpServletResponse.SC_OK).when(httpStatusLine).getStatusCode();
    doReturn(new StringEntity(responseBody, ContentType.TEXT_PLAIN)).when(httpResponse).getEntity();
