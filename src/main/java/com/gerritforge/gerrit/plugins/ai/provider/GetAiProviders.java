...
public class GetAiProviders implements RestReadView<AccountResource> {
  ...
  public record ProviderInfo(String displayName, Set<String> models) {}
  ...
  @Override
  public Response<Map<String, ProviderInfo>> apply(AccountResource resource) throws Exception {
    ...
    Map<String, ProviderInfo> providersPlugins =
        StreamSupport.stream(aiReviewProviders.entries().spliterator(), false)
            ...
            .collect(Collectors.toMap(Extension::getPluginName, this::getProviderInfo));
    return Response.ok(providersPlugins);
  }

  private ProviderInfo getProviderInfo(Extension<AiReviewProvider> ext) {
    AiReviewProvider aiProvider = ext.get();
    return new ProviderInfo(aiProvider.getDisplayName(), aiProvider.getModels());
  }
}
