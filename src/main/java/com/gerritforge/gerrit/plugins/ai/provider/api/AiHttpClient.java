...
public interface AiHttpClient extends HttpClient, Closeable {
  ...
  default Header acceptApplicationJson() {
    return new BasicHeader(HttpHeaders.ACCEPT, APPLICATION_JSON);
  }

  /**
   * Execute an HTTP POST request expecting a 20x status code, with a logic for extracting the error
  ...
  default <T> T post(
    ...
    return execute(
        httpPost,
        s -> s >= HttpStatus.SC_OK && s <= HttpStatus.SC_ACCEPTED,
        errorFromBody,
        responseBodyHandler);
  }

  /**
   * Execute an HTTP GET request expecting a 20x status code, with a logic for extracting the error
   * message from the response body.
   *
   * @param uri the URI of the AI service
   * @param headers the additional HTTP headers for the GET request
