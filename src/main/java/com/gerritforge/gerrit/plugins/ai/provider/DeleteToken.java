...
@Singleton
class DeleteToken implements RestModifyView<AiProviderResource, Input> {
  ...
  public Response<?> apply(AiProviderResource resource, Input input) throws Exception {
    if (resource.getPlugin() == null || resource.getPlugin().isBlank()) {
      throw new BadRequestException("Token provider plugin must be present and non-empty");
    }
    if (!aiReviewProvidersSet.plugins().contains(resource.getPlugin())) {
      throw new BadRequestException(
          "token refers to a non-existent or not-loaded ai-review-provider "
              + resource.getPlugin());
    }
