...
class AiProvidersInfoCacheImpl implements AiProvidersInfoCache {
  ...
  private final Cache<String, LinkedHashSet<String>> modelsCache;

  @Inject
  AiProvidersInfoCacheImpl(
      @Named(AI_MODELS_BY_PLUGIN) Cache<String, LinkedHashSet<String>> aiProvidersModels) {
  ...
  public ProviderInfo getProviderInfo(
    ...
    try {
      return new AiProvidersInfoCache.ProviderInfo(
          ...
          modelsCache.get(pluginName, () -> new LinkedHashSet<>(aiProvider.getModels(apiKey))),
          true);
    } catch (Exception e) {
  ...
  public static CacheModule getModule() {
    return new CacheModule() {
      ...
      protected void configure() {
        persist(AI_MODELS_BY_PLUGIN, String.class, new TypeLiteral<LinkedHashSet<String>>() {})
