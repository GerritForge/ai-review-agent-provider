...
class AiCodeReviewProviderImpl implements AiCodeReviewProvider {
  ...
  async getModels(): Promise<Models> {
    const aiReviewProviders : Map<string,ProviderInfo> = await this.plugin.restApi().get(AI_REVIEW_PROVIDERS_ENDPOINT);

    if (aiReviewProviders.size == 0) {
        return {
                models: [],
                default_model_id: '',
    ...
    const providerModels = [...aiReviewProviders].flatMap(([plugin, providerInfo]) => {
        return providerInfo.models.map((modelName) =>  { return { model_id: `${plugin}/${modelName}`, short_text: providerInfo.displayName,  full_display_text: `${providerInfo.displayName} (${modelName})`}; })
    })

    console.log(`AI Models: ${providerModels}`);
  ...
  private async chatAsync(
    ...
    try {
      ...
      const text = await callAiModelAndGenerateContent({apiKey, model, prompt});
