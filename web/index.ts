...
class GrAiApiToken extends LitElement {
  ...
  @property({type: Array})
  inactiveProviders: ProviderInfo[] = [];
  ...
  async loadProviders() {
    ...
    this.providers = this.providersOutput.providers;
    this.activeProviders = this.providers.filter(p => p.enabled);
    this.inactiveProviders = this.providers.filter(p => !p.enabled);
  ...
  static override get styles() {
    return css`
      ...
      td.commentColumn, td.valueColumn {
          width: 15em;
      }
      h2 {
     ...
     <div class="gr-form-styles">
       ...
       <fieldset id="ai-tokens">
         <table>
           ...
           <tbody>
             ${this.activeProviders.map((provider) => this.renderActiveProvider(provider))}
  ...
  private renderActiveProvider(provider: ProviderInfo) {
    return html` <tr>
      <td class="commentColumn">${provider.display_name}</td>
      ...
      <td class="actionColumn">
        <gr-button
          link=""
          plugin-name=${provider.plugin}
          @click=${(e: Event) => this.deleteToken(e)}
          ?disabled=${this.saving}>Delete</gr-button>
      </td>
    </tr>`;
  }
      ...
      return html`<tr>
           ...
           <td>
              <gr-button
                ...
                ?disabled=${this.saving}>Add</gr-button>
           </td>
        </tr>`;
      } else { return html``; }
  }
  ...
  private async deleteToken(e: Event) {
    const el = e.target as LitElement;
    const plugin = el.getAttribute('plugin-name');
    this.saving = true;
    try {
      await this.plugin.restApi().delete(`${AI_REVIEW_PROVIDERS_ENDPOINT}/${plugin}/apiToken`);
      this.loadProviders();
    } finally {
      this.saving = false;
    }
